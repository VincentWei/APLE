...
# APLEWei), then put it there instead.

macro(APLEWEI_COMPUTE_SOURCES _framework)
...
macro(APLEWEI_INCLUDE_CONFIG_FILES_IF_EXISTS)
...
macro(APLEWEI_ADD_SOURCE_DEPENDENCIES _source _deps)
    set(_tmp)
...
macro(APLEWEI_ADD_PRECOMPILED_HEADER _header _cpp _source)
    if (MSVC)
        ...
        foreach (_src ${_sources})
            APLEWEI_ADD_SOURCE_DEPENDENCIES(${_src} ${PrecompiledBinary})
        endforeach ()
...
macro(APLEWEI_WRAP_SOURCELIST)
    foreach (_file ${ARGN})
        get_filename_component(_basename ${_file} NAME_WE)
        get_filename_component(_path ${_file} PATH)

        if (NOT _file MATCHES "${DERIVED_SOURCES_APLEWEI_DIR}")
            string(REGEX REPLACE "/" "\\\\\\\\" _sourcegroup "${_path}")
            source_group("${_sourcegroup}" FILES ${_file})
        endif ()
    endforeach ()

    source_group("DerivedSources" REGULAR_EXPRESSION "${DERIVED_SOURCES_APLEWEI_DIR}")
endmacro()

macro(APLEWEI_FRAMEWORK_DECLARE _target)
...
macro(APLEWEI_EXECUTABLE_DECLARE _target)
    add_executable(${_target} "${CMAKE_BINARY_DIR}/cmakeconfig.h")
endmacro()

# Private macro for setting the properties of a target.
# Rather than just having _target like APLEWEI_FRAMEWORK and APLEWEI_EXECUTABLE the parameters are
# split into _target_logical_name, which is used for variable expansion, and _target_cmake_name.
# This is done to support APLEWEI_WRAP_EXECUTABLE which uses the _target_logical_name variables
# but requires a different _target_cmake_name.
macro(_APLEWEI_TARGET _target_logical_name _target_cmake_name)
...
macro(_APLEWEI_TARGET_ANALYZE _target)
    if (ClangTidy_EXE)
...
macro(APLEWEI_FRAMEWORK _target)
    _APLEWEI_TARGET(${_target} ${_target})
    _APLEWEI_TARGET_ANALYZE(${_target})
...
# FIXME Move into APLEWEI_FRAMEWORK after all libraries are using this macro
macro(APLEWEI_FRAMEWORK_TARGET _target)
    ...
    add_library(APLEWei::${_target} ALIAS ${_target}_PostBuild)
endmacro()

macro(APLEWEI_EXECUTABLE _target)
    _APLEWEI_TARGET(${_target} ${_target})
    _APLEWEI_TARGET_ANALYZE(${_target})
...
macro(APLEWEI_WRAP_EXECUTABLE _target)
    ...
    _APLEWEI_TARGET(${_target} ${_wrapped_target_name})
    _APLEWEI_TARGET_ANALYZE(${_wrapped_target_name})
...
macro(APLEWEI_CREATE_FORWARDING_HEADER _target_directory _file)
    ...
    # APLEWei2 generate-forwarding-headers script.
...
macro(APLEWEI_CREATE_FORWARDING_HEADERS _framework)
    ...
    if (NOT WIN32)
        ...
        foreach (_currentArg ${ARGN})
            ...
            elseif (_processing_directories)
                ...
                foreach (_file ${_files})
                    APLEWEI_CREATE_FORWARDING_HEADER(${_target_directory} ${_file})
                endforeach ()
            elseif (_processing_files)
                APLEWEI_CREATE_FORWARDING_HEADER(${_target_directory} ${_currentArg})
            endif ()
        endforeach ()
    endif ()
endmacro()

function(APLEWEI_MAKE_FORWARDING_HEADERS framework)
...
function(APLEWEI_COPY_FILES target_name)
    set(options FLATTENED)
...
macro(APLEWEI_DEBUG_DUMP_COMMANDS)
    set(CMAKE_VERBOSE_MAKEFILE ON)
endmacro()

macro(APLEWEI_DEBUG_DUMP_VARIABLES)
...
macro(APLEWEI_ADD_TARGET_PROPERTIES _target _property _flags)
...
macro(APLEWEI_POPULATE_LIBRARY_VERSION library_name)
...
macro(APLEWEI_CREATE_SYMLINK target src dest)
